...
from .adaptive_avgmax_pool import SelectAdaptivePool2d,SNN_SelectAdaptivePool2d
...
class modified_ClassifierHead(nn.Module):
    """Classifier head w/ configurable global pooling and dropout."""
    ...
    def __init__(
        ...
        Args:
            ...
            drop_rate: Pre-classifier dropout rate.
        """
        super(modified_ClassifierHead, self).__init__()
        self.in_features = in_features
        self.use_conv = use_conv
        self.input_fmt = input_fmt
        ...
        self.global_pool = global_pool
        self.pool_if = nn.ReLU() if relu_bool else nn.Identity()

        # self.pool_if = nn.Identity()
        self.drop = nn.Dropout(drop_rate)
        self.fc = fc
        # self.flatten = nn.Flatten(1) if use_conv and pool_type else nn.Identity()
        self.last_fc_if = nn.Identity()
